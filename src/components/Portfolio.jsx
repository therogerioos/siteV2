...
import jsonData from '../json/portfolioResumo.json';
...
function Portfolio() {
    ...
    const endIndex = Math.min(startIndex + itemsPerPage, contentPortfolio.length);

    const itemsForPage = contentPortfolio.slice(startIndex, endIndex);

    useEffect(() => {
        setContentPortfolio(jsonData);
    }, []);

    return (
        <div>
            <section className="section-pages-post-portfolio">
                <h2 className='name-title'>Portfólio</h2>
                <Pagination sx={{
                ...
                }} className='pagination-portfolio' count={Math.ceil(contentPortfolio.length / itemsPerPage)} page={page} onChange={handleChange} />
